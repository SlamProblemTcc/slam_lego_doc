...
Durante a realização de cada ciclo, os artigos foram analisados, avaliados e seus dados foram extraídos como fonte de estudo para a realização deste trabalho de conclusão de curso. Na seção \ref{sub:publicacaoRevisao} serão apresentados os resultados de maneira organizada e simplificada.
	...
	De acordo com as \textit{áreas} destacadas, e com o objetivo de registrar as informações pertinentes obtidas na revisão sistemática, desenvolveu-se a tabela \ref{tab:resultadosRevisao}, onde é apresentada uma contabilização dos resultados obtidos.

	Agora só colocar uma tabela aqui listando os tipos de arquitetura, filtros, informações e os artigos que se encaixam com cada conjunto.
