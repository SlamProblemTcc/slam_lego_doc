...
Durante a realização de cada ciclo, os artigos foram analisados, avaliados e seus dados foram extraídos como fonte de estudo para a realização deste trabalho de conclusão de curso. Na seção \ref{sub:publicacaoRevisao} serão apresentados os resultados de maneira organizada e simplificada.
	...
	\label{sub:publicacaoRevisao}
		...
		Com o objetivo de documentar os resultados diretos desta revisão sistemática, estes resultados foram registrados como as técnicas utilizadas atualmente para solucionar o problema de SLAM em diferentes contextos. Com a identificação e análise destas técnicas, a possibilidade de seleção e adaptação para um contexto simplificado se torna viável.

		Após a realização desta revisão sistemática, foi possível identificar \textit{áreas} mutáveis nas diferentes soluções do problema de SLAM. As áreas identificadas são \textit{arquitetura da solução} \ref{sub:arquitetura_solucao}, \textit{técnica probabilística utilizada} \ref{sub:prob_usada} e \textit{informações disponíveis} \ref{infos_disponiveis}. Em cada solução, os autores buscaram se adequar ao contexto trabalhado, seja a partir da disponibilidade de sensores específicos ou da capacidade computacional disponível.

		Como uma forma de organizar os resultados obtidos, os mesmos serão sub-divididos nas áreas apresentadas acima, apresentando conceitos, vantagens e desvantagens da utilização de cada técnica.
		...
		\label{sub:arquitetura_solucao}
			...
			\begin{figure}[H]
				\centering
				\caption{Exemplo da arquitetura mais utilizada}
				\includegraphics[scale=0.9]{figuras/arquitetura_mais_utilizada.eps}
				\label{img:arquitetura_mais_utilizada}
			\end{figure}

			De acordo com a figura \ref{img:arquitetura_mais_utilizada}, o robô será responsável apenas por obter informações e obedecer as decisões geradas pelo computador. Alguns altores buscaram modificar um pouco esta arquitetura, porém, na maioria dos casos, a mesma foi respeitada. 

			Os artigos que trabalharam com a utilização de multiplos robôs ou aplicações, como o caso de \cite{redeComunicacaoIndustria}, \cite{neuralNetwork} e \cite{multiRobots} também seguiram a mesma arquitetura de comunicação, porém, nestes casos, a obtenção das informações é realizada a partir de múltiplos robôs. Neste tipo de trabalho, os robôs colaboram entre si, pois, como a informação obtida é centralizada em um computador único, as decisões referentes à navegação de um determinado robô são resultados do processamento das informações obtidas por todos os robôs.

			A utilização de múltiplos robôs garante maior eficiência na exploração do ambiente, como afirma \cite{redeComunicacaoIndustria}. Desse modo, a utilização da arquitetura apresentada na figura \ref{img:arquitetura_mais_utilizada} possibilita uma economia computacional, já que apenas uma máquina será responsável por processar todas as informações obtidas, ou seja, viabilizando a utilização de robôs simplificados para obtenção destas informações.
